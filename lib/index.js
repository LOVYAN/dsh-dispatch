...
export class DispatchService extends Service {
	...
	sendJs() {
		return [
			...
			'function preview(form){var input=form.querySelector("input[type=file]");var box=form.querySelector(".thumbs");var hint=form.querySelector(".img-hint");if(!input)return;function draw(){var n=input.files?input.files.length:0;if(hint)hint.textContent=n?("已选 "+n+" 张，最多 20 张"):"可多选，最多 20 张";if(!box)return;box.innerHTML="";if(!n)return;[].slice.call(input.files,0,20).forEach(function(f){var im=document.createElement("img");im.alt=f.name;im.src=URL.createObjectURL(f);box.appendChild(im)})}input.addEventListener("change",draw);draw()}',
			'document.querySelectorAll("form.js-chat").forEach(function(form){preview(form);form.addEventListener("submit",function(ev){',
			...
			'var list=[].slice.call(files.files,0,20);',
			'Promise.all(list.map(function(f){return pack(f).catch(function(){return null})})).then(function(imgs){',
	...
	async handleChatList(urlObj, res) {
		...
		const composer = showArchived ? '' : [
			...
			'<div class="thumbs"></div><p class="muted img-hint">可多选，最多 20 张</p>',
